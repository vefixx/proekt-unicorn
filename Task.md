...
Чтобы собрать квартиры будем группировать по ее номеру, комплексу и строению. Далее, отсортируем строки по дате, и для каждой строки будем смотреть предыдущий час. Если разница текущего часа и предыдущего составляется +30% (или иной процент). При этом исключим "переходы времени суток", то есть уберем переход с утра на день, день на вечер, вечер на ночь и ночь на утро, так как логично, что в эти периоды почти всегда потребление возрастает.
